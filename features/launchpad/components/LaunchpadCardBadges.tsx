      ...
      {chips.map((chip, index) => (
        <Tooltip key={`${chip.label}-${index}`}>
          <TooltipTrigger asChild>
            <div
              className={`flex h-5.25 min-w-14 shrink-0 items-center justify-center gap-1 bg-background rounded-sm border border-(--border) px-1.5 py-0.5 ${chip.val !== undefined ? (chip.val > 50 ? "text-(--chip-green)" : "text-(--chip-red)") : "text-foreground"}`}
            >
              <chip.icon size={12} />
