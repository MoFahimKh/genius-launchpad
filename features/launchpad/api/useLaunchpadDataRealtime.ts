...
export function useLaunchpadDataRealtime({
  ...
  const [launchpadData, setLaunchpadData] = useState<WsRoomUpdateDto["data"] | null>(null);
  ...
  const wsUrl = env.NEXT_PUBLIC_LAUNCHPAD_WS_URL;
  const wsPath = "/data-ws/socket.io";
